...
/**
 * This class has been deprecated as setupPartitions now constructs the global
 * index simultaneously <br>
 * However it is being kept around for a codebase reference
...
public class DeprecatedGIC<T, K, R> extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		FileInputFormat.addInputPath(job, new Path(
				CommandInterpreter.CONSTRUCTFIRSTOUT + "-"
						...
						+ index.getClass().getSimpleName() + "-" + args[3]));
		FileOutputFormat.setOutputPath(job, new Path("Preliminary Global"));

		boolean succ = job.waitForCompletion(true);
		if (!succ) {
			FileSystem fs = FileSystem.get(getConf());
			fs.delete(new Path(CommandInterpreter.CONSTRUCTSECONDOUT + "-"
					+ parser.getClass().getSimpleName() + "-"
					+ index.getClass().getSimpleName() + "-" + args[3]), true);
		}
	...
	@SuppressWarnings("rawtypes")
	public static class GlobalMap<T> extends
			Mapper<HSPIndexNode, HSPLeafNode, Pair<T, IntWritable>, Text> {
		// We only want the first entry from each input file as it contains the
		// root of each local tree
		 ...
		 * throws IOException, InterruptedException { if(key == null){ for(int i
		 * = 0; i < index.partitionPreds.size();i++){ Pair<T,Integer> pred =
		 * ((Pair<T,Integer>) index.partitionPreds.get(i));
		 * if(((WritableComparable
		 * <T>)value.getPredicate()).equals(pred.getFirst())){ context.write(new
		 * Pair(pred.getFirst(), new IntWritable(pred.getSecond())), new
		 * Text(((FileSplit) context.getInputSplit()).getPath().toString())); }
		 * } } else{ for(int i = 0; i < index.partitionPreds.size();i++){
		 * Pair<T,Integer> pred = ((Pair<T,Integer>)
		 * index.partitionPreds.get(i));
		 * if(((WritableComparable<T>)key.getPredicate
		 * ()).equals(pred.getFirst())){ context.write(new Pair(pred.getFirst(),
		 * new IntWritable(pred.getSecond())), new Text(((FileSplit)
		 * context.getInputSplit()).getPath().toString())); } } } }
		 */
	}

	@SuppressWarnings("rawtypes")
	public static class GlobalReducer<T, K, R> extends
			Reducer<Pair<T, IntWritable>, Text, HSPIndexNode, HSPReferenceNode> {
		...
		public void reduce(Pair<T, IntWritable> key, Iterable<Text> values,
				Context context) {
			for (Text pathto : values) {
				Path path = new Path(pathto.toString());
				if (localRoots.size() == 0) {
					localRoots.add(new Pair<Pair<T, Integer>, Path>(
							new Pair<T, Integer>(key.getFirst(), key
									.getSecond().get()), path));
					return;
				}
				for (int i = 0; i < localRoots.size(); i++) {
					if (localRoots.get(i).getFirst().getSecond() > key
							.getSecond().get()) {
						localRoots.add(i, new Pair<Pair<T, Integer>, Path>(
								new Pair<T, Integer>(key.getFirst(), key
										.getSecond().get()), path));
						return;
					}
				...
				localRoots.add(new Pair<Pair<T, Integer>, Path>(
						new Pair<T, Integer>(key.getFirst(), key.getSecond()
								.get()), path));
				return;
			}
		}
			...
			while (localRoots.size() != 0) {
				if (even.size() == 0) {
					for (int j = 0; j < odd.size(); j++) {
						...
						for (int i = 0; i < localRoots.size()
								&& localRoots.get(i).getFirst().getSecond() <= low; i++) {
							for (int k = 0; k < temp.getChildren().size()
									&& (curDep == low || i == 0); k++) {
								if (curDep == low) {
									if (((HSPNode) temp.getChildren().get(k))
											.getPredicate().equals(
													localRoots.get(i)
															.getFirst()
															.getFirst())) {
										...
										localRoots.remove(i--);
										if (localRoots.size() != 0)
											low = localRoots.get(0).getFirst()
													.getSecond();
										break;
									} else if (localRoots.size() == 1
											|| localRoots.get(i + 1).getFirst()
													.getSecond() > low) {
										if (temp.getChildren().get(k) instanceof HSPLeafNode<?, ?, ?>) {
											// temp.children.set(k, new
											// HSPIndexNode<T,K,R>(temp, (T)
											// ((HSPNode)
											// temp.children.get(k)).getPredicate(),index,
											// low+1, curDep));
										} else if (temp.getChildren().get(k) instanceof HSPReferenceNode<?, ?, ?>)
											continue;
										even.add((HSPIndexNode<T, K, R>) temp
												.getChildren().get(k));
									}
								} else if (i == 0) {
									if (temp.getChildren().get(k) instanceof HSPLeafNode<?, ?, ?>) {
								...
								if (curDep == low) {
									if (((HSPNode) temp.getChildren().get(k))
										...
										if (temp.getChildren().get(k) instanceof HSPLeafNode<?, ?, ?>) {
											...
											continue;
										odd.add((HSPIndexNode<T, K, R>) temp
												.getChildren().get(k));
									}
								} else if (i == 0) {
									if (temp.getChildren().get(k) instanceof HSPLeafNode<?, ?, ?>) {
			...
			while (!(stack.size() == 0 && nodule == null)) {
				if (nodule != null) {
					if (nodule instanceof HSPIndexNode<?, ?, ?>) {
						...
						context.write(new HSPIndexNode<T, K, R>(null),
								(HSPReferenceNode) nodule);
						nodule = null;
					}
				} else
					nodule = stack.remove(0);
